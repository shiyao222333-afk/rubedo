...
- 下一步：v0.4.0 平台地基（模块化单体 + SQLite + 集成适配域骨架，见 ARCHITECTURE.md）；v0.4.1 起做环节自动化
...
| v0.4.0 | 🔮 | 平台基建 | **平台地基（架构重构）** | 模块化单体 + SQLite + 集成适配域骨架（见 ARCHITECTURE.md） |
| v0.4.1 | 🔮 | 酷家乐 SOP:2 | **环节自动化** | 逐个环节手动→自动，每换一个量化省时（在地基之上） |
...
| 🟡 P1 | v0.4.1 | 酷家乐 SOP: 逐个环节自动化 | #4 切换验证 + 原则 #2 逐步自动化 |
...
| 数据存储 | **v0.3 及之前：本地 JSON**；**v0.4 起迁 SQLite**（单文件 rubedo.db，见 ARCHITECTURE ADR-001） | 单人工具；SQLite 零部署、可写 SQL、并发安全；存储已隔离，未来换 Postgres 成本低 | 🔄 v0.4 变更 |
...
| 14 | **v0.4 重构为平台地基** | 2026-07-07 经架构评审（docs/ARCHITECTURE.md）确认：v0.4.0 先做模块化单体地基（5 边界 + SQLite + 集成适配域骨架），原"环节自动化"顺延为 v0.4.1 业务功能；存储决策由本地 JSON 改为 v0.4 起迁 SQLite（ADR-001）。 |
