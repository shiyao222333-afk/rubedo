...
def index():
    ...
    window.showDiag = function() {
        ...
        var gridH = (cellH !== '?' && headerH !== '?') ? cellH * 48 + headerH : '?';
        ...
        html += '</table>';
        html += '<p style="margin-top:12px;color:#aaa;font-size:12px;">💡 底部空白>10px = 网格不够高<br>底部空白<-10px = 网格太高</p>';
