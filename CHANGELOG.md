...
### Added
- **事件卡片行内按钮**：每个事件条右侧直接显示完成(✅)和锁定(🔒)按钮，点一下立即执行并切换图标，无需弹窗
- **点击事件弹出编辑菜单**：点击事件弹出"编辑/删除"菜单（替代原来点事件弹详情窗再找按钮的操作），只读事件（节假日/节气）保留详情窗
- **重复事件支持相同操作**：重复事件现在也支持完成/锁定/编辑/删除，情景覆盖自动持久化，刷新后状态保留
- **设置新增事件管理页**：可搜索标题、按分类筛选、删除本周所有事件
- **后端新增**：情景覆盖存储(`occurrence_overrides.json`)，重复事件完成/锁定状态跨刷新保留
